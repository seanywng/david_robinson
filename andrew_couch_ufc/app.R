...
elo_df <- read_csv("C:/Users/seanywng/Documents/R/david_robinson/andrew_couch_ufc/elo_df.csv")
df <- read.csv("C:/Users/seanywng/Documents/R/david_robinson/andrew_couch_ufc/elo.csv")
...
create_elo_data <- function(k) {
  ...
  rbind(temp_df %>% select_at(vars(contains(".A"), 
                                   contains("match_id"))) %>% 
          rename_all(.funs = function(x) str_replace(x, ".A", "")), 
        temp_df %>% select_at(vars(contains(".B"), 
                                   contains("match_id"))) %>% 
  ...
  dashboardBody(
    tabItems(
      tabItem(tabName = "weight_class_tab", 
              box(plotOutput("elo_time_series")), 
...
server <- function(input, output) {
  ...
  output$fighter_card <- renderValueBox({
    ...
    fighter_prob <- round(100*predict(elo, 
                            data.frame(fighter = input$v_fighter,
                                       opponent = input$v_opponent)))
    
    valueBox(value = paste(fighter_prob, "%", sep = ""), 
  ...
  output$opponent_card <- renderValueBox({
    ...
    opponent_prob <- round(100*predict(elo, 
                            data.frame(fighter = input$v_opponent,
                                       opponent = input$v_fighter)))
    
    valueBox(value = paste(opponent_prob, "%", sep = ""), 
             subtitle = paste(input$v_opponent, " Probability", sep = ""), 
             color = "red", 
             icon = icon("hand-rock"))
  })
  
  output$elo_time_series <- renderPlot({
    
    elo_time_series_df <- create_elo_data(input$v_k_1) %>% 
      filter(weight_class == input$v_weight_class_1)
    
    top_5_fighters <- elo_time_series_df %>% 
      filter(weight_class == input$v_weight_class_1) %>% 
      group_by(fighter) %>% 
      arrange(desc(elo)) %>% 
      slice(1) %>% 
    ...
    ggplot(data = elo_time_series_df, aes(x = date, y = elo)) + 
      geom_point(alpha = .5) + 
      geom_point(data = elo_time_series_df %>% filter(fighter %in% top_5_fighters$fighter), 
                 aes(x = date, y = elo, colour = fighter)) + 
  ...
  output$elo_dist <- renderPlot({
    ...
    ggplot(data = elo_dist, aes(x = elo)) + 
      geom_histogram()
  })
}
